...
- The time between points follows an exponential distribution. The distributions differ between the teams.
...
To better explore the point rate, we refer to Figure \@ref(fig:train-observed-tbp-vs-score-plot). It shows the time between points (TBP) for each team as a function of score. As it can be seen by the regression lines, the average TBP is pretty constant throughout the game.
...
Still about the time between points, Figure \@ref(fig:train-observed-tbp) plots the estimated distribution for each team as observed in the train dataset. They have an exponential decay which favors an exponential distribution.
...
Finally, to evaluate the indepndence between points, Figure BLAH.

# Model

Accounting for all the discoveries made in Section [Exploratory data analysis], I came up with the following model. It begins by assuming that each team has a different constant tbp rate, namely $\text{rate}^{\text{blue}}$ and $\text{rate}^{\text{red}}$. This means that independently from team red and its own history, team blue will take $1 / \text{rate}^{\text{blue}}$ minutes on average to make the next point.

It assumes that the time between points are sampled from an Exponential distribution with

```R
# Data
time[blue][1], ..., time[blue][blue_sample_size]
time[red][1], ..., time[red][red_sample_size]

# Transformed data
tbp[blue][i] = time[blue][i] - time[blue][i - 1]
tbp[red][j] = time[red][j] - time[red][j - 1]
...
```{r}
# tar_read(model_tbp_vs_score_plot)
```

```{r}
# tar_read(model_tbp_plot)
```
