...
TODO. Talk about cumulative average model. Almost as good as the bayesian model and more simple. Non null probability for time travel.
...
The model was little better than a simple cumulative average (median error size of `r scales::number(60 * tar_read(test_base_mae), accuracy = 10^(-DECIMALS))` seconds). Therefore, the bayesian model is well suited for a high accurate analysis, but fails to justify its computational demand and math complexity for more sensible applications.
