...
For the last remark, we need to compare how the model performs compared to a baseline. Figure \@ref(fig:test-base-prediction-plot) summarizes the predictions made by a cumulative average. That is, the time of the next point for team blue is simply predicted by `current_time` `+` `blue_average_tbp`. Just like for the bayesian model, it is difficult to distinguish the predictions from the observed data.
...
The model showed a good fit to the data and was able to provide powerful insights such as the probability of team blue winning at any time point. That being said, the performance was little better than a simple cumulative average. Therefore, the bayesian model is well suited for a highly accurate analysis, but fails to justify its computational demand and math complexity for more sensible applications.
