...
**Model and results:** The fit model showed a good fit to the data with test MSE TODO. Moreover, it provided some powerful insights such as credible intervals for the team point rates and the probability of team blue winning at any time point.

**Conclusion:** Unfortunately, the fit model performance was lower than simple cumulative average model which, along with other disadvantages such as computational scale and math complexity make the final model less interesting for robust applications such the ones found in the industry.
...
Finally, we take a look at the correlation between the teams. Figure \@ref(fig:train-observed-window-mean-tbp) exhibits team blue's TBP as a function of team red's TBP, averaged over periods of `r WINDOW_SIZE` seconds. The horizontal regression line indicates that team red has no influence on team blue and vice versa.
